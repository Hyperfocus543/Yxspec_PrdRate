...
```bash
# 显示自动探测到的 PRD 形态（锚点/章节/优先级/列序），不打分
python3 scripts/score_prd.py prd.md --show-profile

# 结构特殊时用 JSON 配置覆盖探测结果（字段见脚本头注释）
python3 scripts/score_prd.py prd.md --config my-config.json
```

---

## 自适应（Adaptive）— 不同模板也能打

**零配置即可对不同模板的 PRD 打分**。脚本启动时自动探测 PRD 形态，无需预设：

| 探测项 | 支持形态 | 示例 |
|---|---|---|
| **锚点** | 任意 `<a id="...">` ID 正则 | `REQ-F-020040` / `FR-0001` / `ID-123` |
| **优先级** | 多套词表自动识别 | `must/should/could`、`P0/P1/P2`、`high/medium/low` |
| **章节** | 按 PRD 实际标题判完整性 | `第1章：文档信息` 或 `功能规格` 均可 |
| **来源** | 从现有链接推断路径特征 | `inputs/`、`src/`、`docs/` |
| **表格列序** | 从表头推断列位置 | 描述/优先级/验收/来源列任意顺序 |

> 结构极端特殊（非表格化 PRD、非标准锚点）时，用 `--config` 覆盖：章节名、锚点正则、优先级词表、来源路径、列序、模糊词等（字段定义见 `score_prd.py` 头部注释）。

**边界说明**：打分面向"表格化结构化 PRD"（含 `<a id>` 锚点 + 需求表格行）。纯段落式 PRD（无锚点无表格）能判 CMP/VER/CLR 等不依赖锚点的维度，但需求条目数 = 0，CON/TRC 无从判定——属工具的设计边界，非缺陷。

---

## 如何工作

1. **自适应探测**：启动时扫描 PRD，识别锚点形态、优先级词表、章节体系、来源路径、表格列序（探测失败回退 yxspec 默认形态）。
2. **解析**：用探测到的锚点正则识别 `<a id="REQ-XXXX">` 锚点行 + 紧随表格行 = 一条需求（精准排除章节编号范围/统计摘要的误识别）。
3. **六维度量**：对每维执行确定性检查（章节覆盖、模糊词正则、优先级枚举、来源行号、编号连续性等）。
4. **加权计分**：按 GQ-rules.yaml 权威权重加权，≥75 判定通过。
5. **对账**：若 PRD frontmatter 已带 `_meta.gq_results.GQ-5`，输出自动分与自评分对比。
...
- **自适应**：自动探测锚点/章节/优先级/来源/列序形态，不同模板零配置可用；特殊结构用 `--config` 覆盖。
...
**Q: 我的 PRD 不是 yxspec 模板（锚点/章节/优先级不同），能打分吗？**
A: 能。工具自适应探测——锚点任意 `<a id>` 格式、优先级 P0/P1 或 must/should、章节按实际标题，均零配置识别。若 PRD 结构极特殊（非表格化、无锚点），用 `--show-profile` 查看识别结果，必要时 `--config` 精确覆盖。

**Q: 纯段落式 PRD（无锚点无表格）能打分吗？**
A: 部分维度可以（CMP 章节覆盖、VER 模糊词、CLR 术语表等），但需求条目数 = 0，CON/TRC 无从判定。打分面向表格化结构化 PRD，这是设计边界。
